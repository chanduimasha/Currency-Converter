...
// const CURRENCIES = ['USD', 'EUR', 'LKR', 'AUD', 'INR'];
...
//         fromCountry: CURRENCY_MAP[fromCurrency].name,
//         toCountry: CURRENCY_MAP[toCurrency].name,
...
//                   {CURRENCY_MAP[currency].flag} {currency} - {CURRENCY_MAP[currency].name}
//                 </option>
//               ))}
//             </select>
...
const CURRENCY_TO_COUNTRY: Record<string, string> = {
  ...
  'AUD': 'Australia',
  'INR': 'India',
  'EUR': 'European Union' // Note: This might not be supported by your API
};
...
// List of supported currencies
const CURRENCIES = ['USD', 'LKR', 'AUD', 'INR']; // Removed EUR as it's not in your backend mapping
...
const CurrencyConverter: React.FC<CurrencyConverterProps> = ({ onTransferComplete }) => {
  const [fromCurrency, setFromCurrency] = useState('USD');
  const [toCurrency, setToCurrency] = useState('LKR'); // Changed default to LKR since EUR is not supported
  const [amount, setAmount] = useState('1.00');
  const [convertedAmount, setConvertedAmount] = useState<number | null>(null);
  const [rates, setRates] = useState<Record<string, number>>({});
  const [loading, setLoading] = useState(false);
